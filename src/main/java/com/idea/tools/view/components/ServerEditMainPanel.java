...
public class ServerEditMainPanel extends JPanel {
	...
	public void fillServer(ServerDto server) {
		...
		server.setLocal(isLocal.isSelected());
	}
	...
	private void render() {
		...
		ActionListener listener = event -> enableButtons();
		...
		nameField.addActionListener(event -> serverEditPanel.getServer().setNameIsAutogenerated(false));
		nameField.getDocument().addDocumentListener(simpleListener(event -> serverEditPanel.getServer().setNameIsAutogenerated(false)));
		nameField.getDocument().addDocumentListener(validator);

		loginField.getDocument().addDocumentListener(validator);
	...
	public void setValues(ServerDto server) {
		...
		isLocal.setSelected(server.isLocal() || "localhost".equals(server.getHost()));
	...
	private void updateNameFieldValue() {
		...
		if (!isLocal.isSelected()) {
			isLocal.setSelected("localhost".equals(host));
		}
	}
